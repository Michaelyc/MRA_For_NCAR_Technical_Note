...
void Approximation::loop_regions_at_finest_level_in_prior(double ***KCholTimesw)
{
    ...
    for(unsigned long iRegion = REGION_START[NUM_LEVELS_M-1]; iRegion < REGION_END[NUM_LEVELS_M-1] + 1; iRegion++)
    {
        ...
        if(nKnotsInCurrentRegion == 0) 
        {
            ...
            if(CALCULATION_MODE=="prediction") get_all_ancestors(ancestorArray[rankOpenMP], iRegion, currentLevel);
        }else
        {
            ...
            for(int jLevel = 0; jLevel < currentLevel; jLevel++)
            {
                ...
                evaluate_cross_covariance(covarianceMatrixMemory[jLevel], partition->knotsX[iRegion], partition->knotsY[iRegion], partition->knotsX[ancestorArray[rankOpenMP][jLevel]], partition->knotsY[ancestorArray[rankOpenMP][jLevel]], nKnotsInCurrentRegion, nKnots, sill, range, nugget);
            }
        ...
        if(CALCULATION_MODE=="prediction")
        {
            ...
            for(int jLevel = 0; jLevel < currentLevel; jLevel++)
            {
                ...
                evaluate_cross_covariance(covarianceMatrixMemory[jLevel],  partition->predictionX[indexRegionAtThisLevel], partition->predictionY[indexRegionAtThisLevel], partition->knotsX[ancestorArray[rankOpenMP][jLevel]], partition->knotsY[ancestorArray[rankOpenMP][jLevel]], nPredictionsInCurrentRegion, nKnots, sill, range, nugget);
